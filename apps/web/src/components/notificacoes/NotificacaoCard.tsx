  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        {/* Ícone */}
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-full ${prioridade.color}`}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <span className={`rounded-full px-2 py-0.5 text-xs ${prioridade.color}`}>
              {tipo.label}
            </span>
            {!isLida && <span className="h-2 w-2 rounded-full bg-blue-500" />}
          </div>

          <h3
            className={`line-clamp-1 font-medium text-gray-800 dark:text-white ${!isLida ? 'font-semibold' : ''}`}
          >
            {notificacao.titulo}
          </h3>
