...
export function useFinanceiro() {
  ...
  // CATEGORIAS
  // ============================================
  const fetchCategorias = useCallback(async (condominioId: string, tipo?: 'receita' | 'despesa'): Promise<CategoriaFinanceira[]> => {
    try {
      let query = supabase.from('categorias_financeiras').select('*').eq('condominio_id', condominioId).is('deleted_at', null).eq('ativo', true).order('codigo');
  ...
  // CONTAS BANCÁRIAS
  // ============================================
  const fetchContas = useCallback(async (condominioId: string): Promise<ContaBancaria[]> => {
    try {
  ...
  const fetchLancamentos = useCallback(async (condominioId: string, filters?: LancamentoFilters) => {
    setLoading(true);
    try {
      ...
      let query = supabase.from('lancamentos_financeiros').select(`*, categoria:categoria_id (codigo, nome), conta_bancaria:conta_bancaria_id (nome_exibicao), criador:criado_por (nome)`, { count: 'exact' })
        .eq('condominio_id', condominioId).is('deleted_at', null).order('data_competencia', { ascending: false }).range(from, to);
  ...
  const createLancamento = useCallback(async (condominioId: string, criadoPor: string, input: CreateLancamentoInput): Promise<LancamentoFinanceiro | null> => {
    setLoading(true);
    try {
      const { data, error: insertError } = await supabase.from('lancamentos_financeiros').insert({ condominio_id: condominioId, criado_por: criadoPor, ...input }).select().single();
  ...
  const updateLancamento = useCallback(async (input: UpdateLancamentoInput): Promise<LancamentoFinanceiro | null> => {
    setLoading(true);
    try {
      const { id, ...updates } = input;
      const { data, error: updateError } = await supabase.from('lancamentos_financeiros').update(updates).eq('id', id).select().single();
  ...
  const deleteLancamento = useCallback(async (id: string): Promise<boolean> => {
    setLoading(true);
    try {
      const { error: deleteError } = await supabase.from('lancamentos_financeiros').update({ deleted_at: new Date().toISOString() }).eq('id', id);
  ...
  const confirmarLancamento = useCallback(async (id: string, aprovadoPor: string): Promise<boolean> => {
    try {
      const { error: updateError } = await supabase.from('lancamentos_financeiros').update({ status: 'confirmado', aprovado_por: aprovadoPor, aprovado_em: new Date().toISOString() }).eq('id', id);
      if (updateError) throw updateError;
  ...
  const getDashboard = useCallback(async (condominioId: string): Promise<DashboardFinanceiro | null> => {
    try {
      const hoje = new Date();
      ...
      return {
        ...
        despesas_por_categoria: Object.entries(despesasPorCategoria).map(([categoria, valor]) => ({
  ...
  return {
    loading, error, lancamentos, pagination,
    fetchCategorias, fetchContas, fetchLancamentos,
    createLancamento, updateLancamento, deleteLancamento, confirmarLancamento,
    calcularSaldoPeriodo, getDashboard
  };
}
