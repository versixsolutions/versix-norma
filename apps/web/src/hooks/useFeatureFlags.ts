...
const toFeatureFlag = (row: any): FeatureFlag => ({
  id: row.id,
  key: row.key ?? row.nome ?? row.id,
  nome: row.nome ?? row.key ?? '',
  descricao: row.descricao ?? null,
  is_enabled: row.is_enabled ?? row.ativo ?? false,
  config: row.config ?? {},
  ambiente: (row.ambiente as FeatureFlag['ambiente']) ?? 'all',
  condominio_ids: row.condominio_ids ?? row.condominios_habilitados ?? null,
...
export function useFeatureFlags() {
  ...
  const fetchFlags = useCallback(async () => {
    ...
    try {
      ...
      setFlags(((data as any[]) || []).map(toFeatureFlag));
    } catch (err) {
  ...
  const createFlag = useCallback(
    async (
      ...
      try {
        ...
        const mapped = toFeatureFlag(data as any);
