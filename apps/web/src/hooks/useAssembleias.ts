...
export function useAssembleias() {
  ...
  const fetchAssembleias = useCallback(async (condominioId: string, filters?: AssembleiaFilters): Promise<AssembleiaComJoins[]> => {
    setLoading(true);
    try {
      let query = supabase.from('assembleias').select('*, convocador:usuarios!assembleias_convocador_id_fkey(nome), secretario:usuarios!assembleias_secretario_id_fkey(nome), pautas:assembleia_pautas(*)').eq('condominio_id', condominioId).order('data_primeira_convocacao', { ascending: false });
      ...
      const transformedData = (data || []).map(item => toAssembleia(item as AssembleiaQueryResult));
  ...
  const getAssembleia = useCallback(async (id: string): Promise<AssembleiaComJoins | null> => {
    try {
      const { data, error: fetchError } = await supabase.from('assembleias').select(`*, convocador:usuarios!assembleias_convocador_id_fkey(nome), secretario:usuarios!assembleias_secretario_id_fkey(nome), pautas:assembleia_pautas(*, opcoes:assembleia_pauta_opcoes(*)), presencas:assembleia_presencas(*, usuario:usuarios!assembleia_presencas_usuario_id_fkey(nome, avatar_url), unidades_habitacionais:unidades_habitacionais!assembleia_presencas_unidade_id_fkey(identificador, bloco:blocos!unidades_habitacionais_bloco_id_fkey(nome)))`).eq('id', id).single();
      if (fetchError) throw fetchError;

      // Buscar quorum
      const { data: quorum } = await supabase.from('v_assembleia_quorum').select('*').eq('assembleia_id', id).single();

      return { ...toAssembleia(data as AssembleiaQueryResult), quorum } as unknown as AssembleiaComJoins;
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : 'Erro desconhecido';
      setError(errorMessage);
      return null;
  ...
  const createAssembleia = useCallback(async (condominioId: string, criadoPor: string, input: CreateAssembleiaInput): Promise<AssembleiaComJoins | null> => {
    setLoading(true);
    try {
  ...
  const updateAssembleia = useCallback(async (input: UpdateAssembleiaInput): Promise<AssembleiaComJoins | null> => {
    setLoading(true);
    try {
  ...
  // Pautas
  const addPauta = useCallback(async (assembleiaId: string, input: CreatePautaInput): Promise<PautaRow | null> => {
    try {
...
export type { AssembleiaFilters, CreateAssembleiaInput, UpdateAssembleiaInput, CreatePautaInput, QuorumInfo };
