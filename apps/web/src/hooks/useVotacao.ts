...
export function useVotacao() {
  ...
  const iniciarVotacaoPauta = useCallback(
    async (pautaId: string): Promise<boolean> => {
      try {
        const { data, error: rpcError } = await (supabase as any).rpc('iniciar_votacao_pauta', {
          p_pauta_id: pautaId,
        });
        if (rpcError) throw rpcError;
        return Boolean(data);
  ...
  const encerrarPauta = useCallback(
    async (pautaId: string): Promise<unknown> => {
      try {
        const { data, error: rpcError } = await (supabase as any).rpc('encerrar_pauta', {
          p_pauta_id: pautaId,
        });
        if (rpcError) throw rpcError;
        return data;
