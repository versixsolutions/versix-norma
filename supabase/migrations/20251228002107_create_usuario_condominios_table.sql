...
CREATE TABLE public.usuario_condominios (
  ...
  role public.role_type NOT NULL DEFAULT 'morador',
  tipo_vinculo public.tipo_vinculo NOT NULL DEFAULT 'proprietario',
  ...
  -- Status
  status public.status_type NOT NULL DEFAULT 'ativo',
...
CREATE INDEX idx_usuario_condominios_ativo ON public.usuario_condominios(condominio_id, status) WHERE status = 'ativo';
