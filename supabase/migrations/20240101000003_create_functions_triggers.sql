...
BEGIN
  ...
  -- Determinar condominio_id
  IF TG_OP = 'DELETE' THEN
    v_condominio_id := CASE 
      WHEN TG_TABLE_NAME = 'condominios' THEN OLD.id
      WHEN OLD.condominio_id IS NOT NULL THEN OLD.condominio_id
      ELSE NULL
    END;
  ELSE
    v_condominio_id := CASE 
      WHEN TG_TABLE_NAME = 'condominios' THEN NEW.id
      WHEN NEW.condominio_id IS NOT NULL THEN NEW.condominio_id
      ELSE NULL
    END;
  END IF;
